...
public class Main {
	...
	public static void main(String[] args) {
		...
		final int  BORAD_SIZE = Board.ROWS*Board.COLUMNS;
		int XColumnPosition;
		int maxDepth = 4;
		MinimaxAI XPlayer = new MinimaxAI(maxDepth, Board.X);
		NeuralNetwork OPlayer = 
				new NeuralNetwork(BORAD_SIZE, Board.COLUMNS,500) ;
		...
		while(!connect4.checkGameOver()) {
			...
			switch (connect4.getLastSymbolPlayed()) {
				...
				case Board.O:
                    System.out.print("MinMax 'X' moves:");
                    //minimax move
					Move XMove = XPlayer.MiniMax(connect4);
...
//        				break;
//        			}
					connect4.makeMove(XMove.getCol()-1, Board.X);
					System.out.println();
					break;
				...
				case Board.X:
                    System.out.println("Neural Network 'O' moves:");
                    
                    Move OMove =OPlayer.NeuralNetworkMove(connect4);
					// random move
					//Random r = new Random();
